...
(() => {
  ...
  ].map(j=>({...objects.find(o=>o.id===j.object_id),...j,object_code:objects.find(o=>o.id===j.object_id).code,object_name:objects.find(o=>o.id===j.object_id).name,bonus:0}));
  const cleaners=[{id:3,full_name:'Demo cleaner',email:'cleaner@example.invalid',active:true,mode:'FLEX',transport:'CAR',reliability_score:96,rating:4.9,today_jobs:1,completed_jobs:42,cancellation_rate:0,reclean_rate:0,avg_delay_minutes:2,preferred_zones:['Bratislava — Staré Mesto','Praha — Nové Město']}];
  const clients=[{id:4,full_name:'Demo owner',company_name:'Demo stays',account_type:'OWNER',email:'owner@example.invalid',user_active:true,active:true,month_revenue:175},{id:5,full_name:'Demo manager',company_name:'Demo stays Prague',account_type:'PROPERTY_MANAGER',email:'manager@example.invalid',user_active:true,active:true,month_revenue:100}];
  ...
  const finance=month=>({month,from:month+'-01',to:today,summary:{completedJobs:1,totalRevenue:42,cleaningRevenue:42,manualIncome:0,cleanerPayouts:24,cleanerBonuses:0,jobExtraCosts:0,manualExpenses:0,totalExpenses:24,profit:18,marginPct:43,avgProfitPerJob:18,avgRevenuePerJob:42},trend:[{month,revenue:42,expenses:24,profit:18}],byObject:[{code:'BA-001',name:'Old Town apartment',jobs:1,revenue:42,cost:24,profit:18}],byClient:[{id:4,name:'Demo stays',revenue:42}],entries:[]});
  const settlements=()=>({summary:{chargedCents:4200,receivedCents:0,dueCents:4200,earnedCents:2400,paidCents:0,payableCents:2400},jobs:[],hasMore:false});
